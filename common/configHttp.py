...
class RunMain():
    ...
    def send_post(self,url,data):   #定义一个方法，传入需要的参数url和data
        #参数必须按照url,data顺序传入
        result = requests.post(url = url,data = data).json()    #因为这里封装post方法，所以这里的url和data不能写死
...
if __name__=='__main__':    #能过写死参数，来验证请求是否正确
    result1 = RunMain().run_main('post','http://127.0.0.1:8888/login',{'name':'xiaoming','pwd':'111'})
    result2 = RunMain().run_main('get','http://127.0.0.1:8888/login','name=xiaoming&pwd=')
    print(result1)
    print(result2)
